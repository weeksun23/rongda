...
Vue.component('v-accordion', {
  ...
  methods : {
    ...
    //选中item
    selectItem : function(ch,unTrigger){
    ...
    //根据text选取item
    selectItemByTitle : function(title,unTrigger){
      this.findItem(function(item,itemIndex,panel,panelIndex){
        if(item.title === title){
          this.curIndex = panelIndex;
          this.selectPanel(panelIndex);
          this.selectItem(item,unTrigger);
          return true;
        }
      });
    }
